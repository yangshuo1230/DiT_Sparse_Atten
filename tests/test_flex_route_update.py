...
def test_periodic_route_update_replaces_block_mask(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda include_grid=False: (1, 1, 0, None)
        if include_grid else (1, 1, 0))
    ...
    backend.state[(1, 0, tokens, heads, head_dim, q.device, None)] = {
...
def test_spatial_bootstrap_restores_original_token_order(monkeypatch):
    ...
    monkeypatch.setattr(
        flex_module, "routing_context",
        lambda include_grid=False: (1, 0, 0, (1, 4, 64))
        if include_grid else (1, 0, 0))
    torch.manual_seed(8)
...
def test_first_frame_sink_is_added_to_every_query_row():
    backend = FlexReuseBackend(FlexReuseConfig(block_size=128))
    route = torch.zeros(2, 4, 4, dtype=torch.bool)
    result = backend._add_first_frame_sink(
        {"grid": (2, 8, 16), "layout": None}, {"route": route})
    assert result["route"][..., 0].all()
    assert not result["route"][..., 1:].any()


def test_first_layer_is_always_dense(monkeypatch):
    import attention_backends.flex as flex_module

    monkeypatch.setattr(
        flex_module, "routing_context",
        lambda include_grid=False: (0, 3, 1, (2, 2, 2))
        if include_grid else (0, 3, 1))
    q = torch.randn(1, 8, 2, 4)
    k = torch.randn_like(q)
    v = torch.randn_like(q)
    backend = FlexReuseBackend()
    output = backend(q, k, v, dtype=torch.float32)
    reference = torch.nn.functional.scaled_dot_product_attention(
        q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2),
    ).transpose(1, 2)
    ...
    assert backend.phase_counts["first_layer_dense"] == 1
